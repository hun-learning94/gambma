...
Rcpp::List gambmsVS(const arma::vec &y,
  ...
  arma::vec knots, knots_tmp = arma::vec();
  arma::uvec knotsidx, knotsidx_tmp = arma::uvec();
  arma::vec qts; double bdmargin{.0};
  arma::uvec maxk_adj{maxk};
  
  for(unsigned i{0}; i<P; i++){
    qts = arma::linspace(bdmargin, 1.0 - bdmargin, maxk(i)); // all predictors rescaled to [0,1]
    knots_tmp = arma::unique(arma::quantile((X.col(i)), qts));
    // knots_tmp = arma::quantile(arma::unique((X.col(i))), qts);
    knotsidx_tmp = arma::uvec(knots_tmp.n_elem, arma::fill::value(i+1));
    knots = arma::join_cols(knots, knots_tmp);
    knotsidx = arma::join_cols(knotsidx, knotsidx_tmp);
    maxk_adj(i) = knots_tmp.n_elem;
  }
  ...
  while(!initialdrawn){
    ...
    try{
      MATX_TO_LPY(lpyCurr, comp1Curr, comp2Curr, comp3Curr, r2QmCurr,
                  ...
                  Rglm, true, forceLin, linProb, maxk_adj);
      initialdrawn = true; break;
    } catch(...) {
      initialtry++;
      if(initialtry % 100 == 0){ Rcpp::Rcout << "Initial failed for " << initialtry << " times \n"; }
  ...
  for(unsigned i{0}; i<MCMCiter; i++){
    ...
    for(unsigned j{0}; j<P; j++){
      ...
      // Rcpp::Rcout << "  sItr is " << sItr << "\n";
      ssPermu = arma::randperm(maxk_adj(sItr));
      ssHlpr = 0; for(unsigned l{0}; l < sItr; l++){ ssHlpr += (maxk_adj(l) + 1); }
      start = ssHlpr; end = ssHlpr + maxk_adj(sItr) - 1; // set of knots for current variable
      
      for(unsigned k{0}; k<maxk_adj(sItr); k++){
        ssCnt++;
        ssItr = ssPermu(k); // current knot
        // Rcpp::Rcout << "    ssItr is " << ssItr << "\n";
