...
arma::mat CRADNS_1d_cpp(const arma::vec& x, const arma::vec &knot, 
  ...
  if(knotalive){
    ...
    B.set_size(N, nk - 1);
    B.each_col() = (-armapmax(arma::pow(arma::abs(x - augknot(nk-1)), 3), 0) + armapmax(arma::pow(arma::abs(x - augknot(0)), 3), 0)) / 
      (augknot(nk-1) - augknot(0));
    for(unsigned i{0}; i < (nk-2); i++){
      B.col(i + 1) -= (armapmax(arma::pow(arma::abs(x - augknot(nk-1)), 3), 0) - armapmax(arma::pow(arma::abs(x - augknot(i)), 3),0)) / 
