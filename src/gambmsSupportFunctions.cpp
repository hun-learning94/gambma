...
void knotPrior(double &comp3,
  ...
  for(unsigned i{0}; i<Lambda.n_elem; i++){
    ...
    if(isgrid) {
      // Rcpp::Rcout << "i " << i << " maxk(i) " << maxk(i) << " knotnums " << knotnums << "\n";
      comp3 -= 
...
void MATX_TO_SAMPLE(arma::vec &FittedSmooths,
  ...
  gPosteriors(g_, v_, t_, q_, l_, m_,
              static_cast<double>(N), static_cast<double>(mle.n_elem)-1.0, 
  ...
  PredLinears(0) = alpha; // 11122023 need to adjust alpha 
  ...
  // 11122023 need to adjust alpha 
  if((familyLink / 10) > 1){
	alpha -= alphaAdj(JEtaHat, beta, mle.subvec(1, mle.n_elem-1), B_tr); 
	PredLinears(0) = alpha;  
  }
...
double alphaAdj(const arma::vec &JEtaHat,
				...
				const arma::mat &B_tr){
  arma::vec betaB = B_tr.cols(1, B_tr.n_cols - 1) * (beta-betahat);
  return(arma::dot(betaB, JEtaHat)/arma::accu(JEtaHat));
  // arma::vec etaB = arma::trans(JEtaHat.t() * B_tr.cols(1, B_tr.n_cols - 1));
  // return(arma::dot(etaB, beta)/arma::accu(JEtaHat));
}
