...
METHODS = c("Even-knot","Free-knot","VS-knot")
...
colnames(ESS_all) = c("Even-knot", "VS-knot", "Free-knot")

datdir = paste0("results/", SIMNAME ,"/", family ,"/n", tail(N,1), "/f", paste0(target, collapse = ""))
ESS_all = read.csv(paste0(datdir, "/ESS.csv"))

pdf(paste0(datdir, "/ESS.pdf"), 
    width = 1.8*3, height = 1.8*1)
par(mar=c(1.5, 2.5, 1.5, 1), mgp=c(1.5,0.5,0), 
    oma = c(0,0,0,0), cex.axis =1., cex.main= 1.)
par(mfrow=c(1,3))
boxplot(ESS_all[1:500,2:3], col = "skyblue", main = paste0("n = ",N[1]),
        ylab = "ESS/Time")
boxplot(ESS_all[1:500+500,2:3], col = "skyblue", main = paste0("n = ",N[2]),
        ylab = "ESS/Time")
boxplot(ESS_all[1:500+1000,2:3], col="skyblue", main = paste0("n = ",N[3]),
        ylab="ESS/Time")
dev.off()
