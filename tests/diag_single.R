      ...
      for(i in 1:(itermax*2)){
        ...
        if(!file.exists(filename)){
          ...
          dat = simmat(f_list, xrange[1], xrange[2], n, family, link, nbinom = nbinom, sig = sig,
                       intercept = 0,
                       xintv = c(xrange[1], xrange[1] + 0.5, xrange[2] - 0.5, xrange[2]),
                       xprob = c(0.1, 0.8, 0.1))
          X = as.matrix(dat[,-c(1)])
          mins = apply(X, 2, "min")
          maxs = apply(X, 2, "max")
        ...
        if(i_done %% iter_mse == 0 & startmid ==0){
          ...
          png(paste0(resultdir_sum, "/summary_",cnt,  ".png"), width = 1000, height = 200*p)
          ...
          for(whichmethod in METHODS){
            ...
            for(p in seq_along(target)){
              ...
              lines(PREDTABLE$grid[PREDTABLE$id == 1 & crit], apply(matrix(PREDTABLE$mean[crit], byrow = T, nrow = cnt), 2, mean),
                    col = "blue", lty =2)
