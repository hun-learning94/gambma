...
plot(g, hyperg(g, log=T), type = "n", main = "g=n", 
     ylab = TeX("$\\log\\; p(g)$"), xlab = TeX("$g$"), ylim=ylim)
for(i in seq_along(N)){ abline(v = N[i], lty=i, col = i+1)}
# legend("topright", legend = paste("n", c(N)), lty = 1:(length(N))+1, lwd = 1.5, col = 1:length(N)+1)
...
plot(g, uniform(g, log=T), type = "n", main = "Uniform",  ylab = TeX("$\\log\\; p(g)$"), xlab = TeX("$g$") , ylim=ylim)
for(i in seq_along(N)){ lines(g, uniform(g, log=T), lty=i, col = i+1);  }
# legend("topright", legend = paste("n", c(N)), lty = 1:(length(N)), lwd = 1.5, col = 1:length(N)+1)

plot(g, hypergn(g, n, log=T), type = "n", main = "Hyper-g/n",  ylab = TeX("$\\log\\; p(g)$"), xlab = TeX("$g$"), ylim=ylim)
...
plot(g, truncZS(g, n, log=T), type = "n", main = "ZS-adapted",  ylab = TeX("$\\log\\; p(g)$"), xlab = TeX("$g$") , ylim=ylim)
for(i in seq_along(N)){ lines(g, truncZS(g, N[i], log=T), lty=i, col = i+1);  }
# legend("topright", legend = paste("n", c(N)), lty = 1:(length(N)), lwd = 1.5, col = 1:length(N)+1)

plot(g, betaprime(g, n, p, log=T), type = "n", main = "Beta-prime",  ylab = TeX("$\\log\\; p(g)$"), xlab = TeX("$g$") , ylim=ylim)
for(i in seq_along(N)){ lines(g, betaprime(g, N[i], p, log=T), lty=i, col = i+1);  }
# legend("topright", legend = paste("n", c(N)), lty = 1:(length(N)), lwd = 1.5,col = 1:length(N)+1)

plot(g, robust(g, n, p, log=T), type="n", main = "Robust",  ylab = TeX("$\\log\\; p(g)$"), xlab = TeX("$g$") , ylim=ylim)
for(i in seq_along(N)){ lines(g, robust(g, N[i], p, log=T), lty=i, col = i+1)};
# legend("topright", legend = paste("n", c(N)), lty = 1:(length(N)), lwd = 1.5, col = 1:length(N)+1)

plot(g, intrinsic(g, n, p, log=T), type="n", main = "Intrinsic",  ylab = TeX("$\\log\\; p(g)$"), xlab = TeX("$g$") , ylim=ylim)
