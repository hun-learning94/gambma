...
for(n in N){
  ...
  for(i in 1:Nsim_max){
    ...
    tic = Sys.time()
    fit = tryCatch(
      gam(y~ s(x1, bs = basis, k = maxk) + 
            s(x2, bs = basis, k = maxk) +
            s(x3, bs = basis, k = maxk),
          data = dat, family = familybayesX),
      error = function(cnd)cnd
    )
    toc = Sys.time()
    ...
    tmp = plot_mgcv(fit, Xgrid, f_list, 0.95, maxk, elapsed = elapsed, ylim = ylim,
                    Linadj = linadj)
