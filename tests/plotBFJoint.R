...
gpriors = 1:8
PRIOR = c("Unit Information (g=n)", "Uniform","Hyper-g", "Hyper-g/n", "Beta-prime", "ZS-adapted","Robust", "Intrinsic")
priorCode = Vectorize(function(prior){
  # c("Unit", "Hyper-g", "Uniform", "Hyper-g/n", "Beta-prime", "ZS-adapted", "Robust", "Intrinsic", "constant", "CH")
  switch(prior, 
         "Unit Information (g=n)" = "unit", "Hyper-g" = "hyperg", "Uniform" = "uniform", "Hyper-g/n" = "hypergn", "Beta-prime" = "betaprime", 
...
par(mar=c(4, 5, 2.5, 1/2), mgp=c(3.2,1,0), oma = c(0,0,0,0), 
    cex.axis = 1.8, cex.main= 1.8, cex.lab=1.8)
layout(matrix(c(1,2,3,4, 9,
                5,6,7,8, 10), byrow=T, nrow=2), widths = c(rep(1,4), "1.6 cm"))
...
par(mar=c(3.3, 3.3, 2.5, 1/2),mgp=c(2.2,0.5,0), oma = c(0,0,1.5,0), cex.axis = 1.5, cex.main= 1.5, cex.lab=1.5)
layout(matrix(c(1,2,3,4, 9,
                5,6,7,8, 10), byrow=T, nrow=2), widths = c(rep(1,4), "1.6 cm"))
for(i in seq_along(plotlist)){
  filledContour(R2, pp, t(plotlist[[i]]), zlim=zlim,
