...
ELAPSED_all = matrix(0, nrow = length(N)*(Nsim_max-1), ncol = length(METHODS))
fuck = 0
  ...
  for(n in N){
    ...
    for(i in 1:Nsim_max){
      ...
      for(ffuck in seq_along(METHODS)){
        ...
        ELAPSED_all[fuck, ffuck] = as.numeric(elapsed)
      }
    }
  }
}

colnames(ELAPSED_all) = METHODS
par(mfrow=c(1,3))
boxplot(ELAPSED_all[1:500,])
boxplot(ELAPSED_all[1:500+500,])
boxplot(ELAPSED_all[1:500+1000,])

write.csv(ELAPSED_all, row.names = F, 
          file = paste0(datdir, "/ELAPSED.csv"))
