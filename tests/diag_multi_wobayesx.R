...
for(target in TARGET){
  ...
  method5 = "free"
  method7 = "blapsr"
  METHODS = c(method1, method2, method3, method4, method5, method7)
  # METHODS_col = c("palevioletred1", "deeppink1", "steelblue1", "turquoise3", "slateblue1", "limegreen", "olivedrab")
  METHODS_col = c("palevioletred1", "deeppink1", "steelblue1", "turquoise3", "slateblue1", "olivedrab")
    ...
    for(sig in Sig){
      ...
      for(i in 1:(itermax*2)){
        ...
        if(!file.exists(filename)){
          ...
          X = as.matrix(dat[,-1])
          linterms = grepl("lin_", colnames(X), fixed=T)
          X = X[, !linterms]
          mins = apply(X, 2, "min")
          maxs = apply(X, 2, "max")
          ...
          tic = Sys.time()
          fit7 = tryCatch(
            gamlps(y ~ sm(x1)+sm(x2)+sm(x3) +sm(x4),
          ...
          tic = Sys.time()
          fit3 = tryCatch(
            gambma(y ~ sm(x1) + sm(x2) + sm(x3) + sm(x4), dat, 
                   maxk = maxk, 
          ...
          fit4 = tryCatch(
            gambma(y ~ sm(x1) + sm(x2) + sm(x3) + sm(x4), dat, 
                   maxk = maxk, 
                   knot_config = method4, prior = "betaprime", family = family, link = link,  
                   nbinom = nbinom,
                   g_manual = g_manual,
                   lambda = lambda,
                   MCMCiter = MCMCiter,
          ...
          fit5 = tryCatch(
            gambma(y ~ sm(x1) + sm(x2) + sm(x3) + sm(x4), dat, 
                   maxk = maxk, 
                   knot_config = method5, prior = "betaprime", family = family, link = link,  
                   nbinom = nbinom,
                   g_manual = g_manual,
                   lambda = lambda,
                   MCMCiter = MCMCiter,
          ...
          ###############################################################################
          PREDtable = rbind(tmp1, tmp2, tmp3,tmp4, tmp5, tmp7)
          save(fit1, elapsed1,
               fit2, elapsed2,
               fit3, elapsed3,
               fit4, elapsed4,
          ...
          if(!exists("fit7")){
            ...
            tic = Sys.time()
            fit7 = tryCatch(
              gamlps(y ~ 1+sm(x1)+sm(x2)+sm(x3)+sm(x4)+sm(x5)+sm(x6),
                     data = dat, K = maxk, penorder = degree - 1, family=blapsr_family),
              error = function(cnd)cnd
            )
            toc = Sys.time()
          ...
          #              sx(x5, bs = "ps", degree = degree, knots = maxk)+
          #              sx(x6, bs = "ps", degree = degree, knots = maxk),
        ...
        if(T){
          ...
          save(fit1, elapsed1,
               ...
               fit7, elapsed7,
               PREDtable, file = filename)
          rm(fit1, fit2, fit3,  fit4, fit5, fit7)
        }
        ...
        if(i_done %% iter_mse == 0 & startmid ==0){
          ...
          for(whichmethod in METHODS){
            ...
            if(p==1) par(mfrow=c(1,1)) else par(mfrow=c(2,length(target)/2))
