...
plotBFMarg = function(likelihood, gpriors, cols, ltys,
  ...
  par(mar=c(5, 3, 2.5, 2),
      mgp=c(3.5,1,0), 
      oma = c(0,0,0,0), 
      cex.axis = 1.6, cex.main= 1.8, cex.lab=1.8)
  plotpos = 0
    ...
    for(j in seq_along(yaxis)){
      ...
      if(!missing(legendpos)){
        if(plotpos == legendpos)
          legend("topright", legend = plotgpriors(gpriors), lty = ltys, lwd = 3, col = cols, ncol=2,
                                        cex=1.4, seg.len=1.5, text.width = 12, xjust=0.5)
      }
...
tmp1 = plotBFMarg(likelihood, gpriors, cols, ltys,
               xaxis = R2_xaxis, xlab = "R2pseudo", yaxis = pp_yaxis, ylab = "p", diffplots = N, plotname = "N",ylim= c(0,4), 
               xtickloc = nplot_r2*r2tickloc, xticklabel = r2tickloc)
tmp1plot = recordPlot()
tmp2 = plotBFMarg(likelihood, gpriors, cols, ltys,
                xaxis = pp_xaxis, xlab = "p", yaxis = R2_yaxis, ylab = "Qm", diffplots = N, plotname = "N",ylim= c(0,4),1,
...
pdf(paste0(plotdir,"log_BF_exp_r2pseudo.pdf"),  width=10, height = 9)
tmp1plot
dev.off()
pdf(paste0(plotdir,"log_BF_exp_p.pdf"),  width=10, height = 9)
tmp2plot
dev.off()
pdf(paste0(plotdir,"log_BF_norm_r2.pdf"), width=10, height = 9)
tmp1plot_gau
dev.off()
pdf(paste0(plotdir,"log_BF_norm_p.pdf"),  width=10, height = 9)
tmp2plot_gau
dev.off()
