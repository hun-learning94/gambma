...
for(j in seq_along(N)){
  ...
  for(target in seq_along(TARGET[[1]])){
    ...
    par(mar=c(1.5, 2, 1.5, 1), mgp=c(1.5,0.5,0), oma = c(0,0,1,0), cex.axis = .9, cex.main= 1.1)
    par(mfrow=mfrow)
    for(method in METHODS){
...
for(pp in seq_along(TARGET[[1]])){
  ...
  if(pp!=3) legend("topright", col = METHODS_col, bty="n", lwd = 5, legend = METHODS, ncol=2, cex = 2)
